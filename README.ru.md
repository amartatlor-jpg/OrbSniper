...
<tr>
<td width="50%">
<img src="docs/screenshot-working.png" alt="OrbSniper фармит квесты">
<p align="center"><b>В работе</b><br>Прогресс, счёт и консоль, которая говорит по-человечески</p>
</td>
<td width="50%">
<img src="docs/screenshot-finish.png" alt="Все квесты выполнены">
<p align="center"><b>Готово</b><br>Скажет, что сделано и что осталось забрать вручную</p>
</td>
</tr>
</table>

<div align="center">
<img src="docs/screenshot-disclaimer.png" alt="Предупреждение при каждом запуске" width="55%">
<p><b>Каждый запуск начинается отсюда</b> — риски, правило Discord и галочка, которую надо поставить</p>
</div>
...
Скачай `OrbSniper.exe` из [релизов](../../releases/latest) и запусти. Node.js на машине не нужен.

При каждом запуске показывается экран с рисками — прочитай, поставь галочку, продолжай.
...
Прогнать проверки (Discord и сеть не нужны):
...
| Кнопка | Что делает |
|---|---|
| **Начать** | Проверяет условия, при необходимости перезапускает Discord, впрыскивает логику, начинает фарм |
| **Стоп** | Прекращает всё немедленно. Discord остаётся открытым |
| **Пропустить** | Бросает текущий квест и переходит к следующему |
| **Починить и перезапустить** | Появляется, если что-то сломалось. Закрывает все процессы Discord намертво, освобождает порт и начинает заново |
...
**У друга работает, а у меня нет.**
Ради этого и делалось это обновление. Нашлось шесть причин, каждая из которых не проявляется на машине разработчика и встречается у остальных: оставшаяся папка `app-*` заставляла запускать устаревшую сборку Discord, искалась только стабильная ветка, занятый другим Electron-приложением порт `9222` считался фатальной ошибкой, движок опирался на минифицированные имена, которые отличаются между сборками Discord, путь к `AppData` был зашит жёстко и ломался при перенесённом профиле, а подсказка про запуск от администратора не показывалась на не-английской Windows: ответ `taskkill` приходит в другой кодировке, и программа его не распознавала. Все шесть закрыты: порт подбирается автоматически, PTB и Canary находятся, берётся новейшая сборка, а внутренности Discord ищутся по структуре, а не по имени.

Если всё равно не идёт — нажми **«Починить и перезапустить»** и пришли лог из консоли (кнопка **«Копировать»** забирает его целиком).

**У меня Discord PTB или Canary.**
Поддерживаются, как и Discord, установленный в нестандартное место — если он запущен, берётся его настоящий путь.
